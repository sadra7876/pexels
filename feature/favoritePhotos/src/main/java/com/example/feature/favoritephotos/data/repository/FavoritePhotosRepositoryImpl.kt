...
import com.example.feature.favoritephotos.data.mapper.toFavoritePhotoListDN
...
internal class FavoritePhotosRepositoryImpl(
    ...
    override fun getFavoritePhotos(): Flow<PagingData<FavoritePhotoListDN>> {
        return Pager(
            config = PagingConfig(pageSize = 20),
            pagingSourceFactory = { favoriteDao.getFavorites() }
        ).flow
            .map { pagingData ->
                pagingData.map { it.toFavoritePhotoListDN() }
            }
    }
}